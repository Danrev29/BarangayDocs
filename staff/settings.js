...
import { setDoc } from "https://www.gstatic.com/firebasejs/10.7.2/firebase-firestore.js";
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  onAuthStateChanged(auth, async (user) => {
    if (user) {
      ...
      const uid = user.uid;
      const adminDocRef = doc(db, "users", uid); // fetch from 'admins' collection
      ...
      try {
        ...
        if (docSnap.exists()) {
          ...
          document.getElementById("phone").textContent = adminData.phone || "N/A"; // optional
        } else {
